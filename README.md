...
# Также игнорируются столбцы, название которых начинается с символа _
IGNORE_COLUMNS=
```
...
При выводе не показываются: столбец с серийным номером; столбцы, индексы которых указаны в `IGNORE_COLUMNS`; столбцы, название которых начинается с символа `_` (например, `_internal_note`).
